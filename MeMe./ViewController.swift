   ...
   override func viewWillAppear(_ animated: Bool) {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            imagePikerView.contentMode = .scaleToFill
    ...
    func save(memedImage: UIImage){
        // Create the meme
        let meme = MeMe(topText: topTextField.text!, bottomText: buttomTextField.text!, originalImage: imagePikerView.image!, memedImage:memedImage)
    ...
    @IBAction func shareAction (){
        ...
        let activity = UIActivityViewController(activityItems: [memeShare], applicationActivities: nil)
        activity.completionWithItemsHandler = { (activity, success, items, error) in
            if success {self.save(memedImage: memeShare)}
        }
